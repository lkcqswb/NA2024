...
int main(){
    ...
    vector<double> testcase={-3.5,-3,-0.5,0,0.5,3,3.5};
    ...
    ofstream outfile("Pd.txt", ios::trunc);
    for (double t : testcase) outfile << t << " " << fabs(f(t)-bs23.get_value(t)) << endl;
    outfile << "#END# " <<"B^2_3"<< endl;
    for (double t : testcase) outfile << t << " " << fabs(f(t)-bs12.get_value(t)) << endl;
    outfile << "#END# " <<"B^1_2"<< endl;
    string command = "python plot.py Pd.txt";
